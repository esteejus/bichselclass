...
void fcn(int& npar, double* deriv, double& f, double par[], int flag){
  ...
  double lnL = 0.0;
    ...
    double s_dedx = d_dedx*par[1]+par[0]; //scaled dedx
...
int main(){
  ...
  setdedx_datafile("./data_anode14_x10.dat");
 ...
 // Initialize minuit, set initial values etc. of parameters.
  ...
  par[0] = .4;            // a guess at the true value.
  ...
  par[1] = .04;            // a guess at the true value.
  ...
  outpar[1]=.025;
  outpar[0]=.9;
    TH1D *s_result = new TH1D("s_result","s_result",2000,0,20);
    TH1D *expected = new TH1D("expected","expected",2000,0,20);
  
  for(int i=0;i<data_dedx.size();++i){
    s_result->Fill(outpar[1]*data_dedx.at(i)+outpar[0]);
  }
  for(int i=0;i<t_dedx.size();++i){
    int bin = expected->FindBin(t_dedx.at(i));
    expected->SetBinContent(bin,t_value.at(i));
  }

  TCanvas *c1 = new TCanvas(1);
  //  c1->SetLogy();
  s_result->SetLineColor(2);
  expected->Scale(1./expected->Integral());
    expected->Draw();
    s_result->Scale(1./s_result->Integral());
      s_result->Draw("same");
