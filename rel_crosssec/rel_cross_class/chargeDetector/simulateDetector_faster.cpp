  ...
  for(int iLayer = 1; iLayer<= num_layers; iLayer++)
    {
      ...
      auto itt = remove_if(padplane.at(iLayer).begin(),padplane.at(iLayer).end(),[](double x){ return x < 1e-2;});
      curLayer.erase(itt,padplane.at(iLayer).end());
...
int main()
{
  ...
  //  vector<vector<double>> padplane[num_mc];
      ...
      for(int iSeg = 0 ;iSeg < ns; iSeg++)
	{
	  ...
	  //	  double num_elec  = floor(split_chg/eVperelec);
    ...
    //    cout<<"theory bins "<<   c_dist->GetNbins();
    //    cout<<"det reponse bins "<<   c_dist_p10->GetNbins();
    TFile *root_out = new TFile("cstrag_out.root","RECREATE");
    c_dist->SetName("detreponse");
    c_dist->Write();
    c_dist_p10->SetName("theory");
    c_dist_p10->Write();
    root_out->Close();
    
    double diff = c_dist->GetMean() - c_dist_p10->GetMean();
    cout<<"Diff in mean is "<<diff<<endl;
 ...
 ofstream cfile;
  ...
  // bichsel_p10->Draw();
    c_dist->Scale(1./c_dist->Integral("width"));
        c_dist->GetXaxis()->SetRangeUser(1e3,10e3);
    c_dist->Draw();
