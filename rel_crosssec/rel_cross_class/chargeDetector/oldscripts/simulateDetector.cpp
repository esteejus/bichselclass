...
struct hit {
  int row;
  int layer;
  double charge;//or charge fraction
};
...
bool by_double(const double &a, const double &b)
{
  return(a < b);
}

bool by_layer_row(const hit &a, const hit &b)
{
  if(a.layer != b.layer)
...
bool by_layer(const hit &a, const hit &b)
{
    return (a.layer < b.layer);
}
...
vector<vector<double>> InitPadPlane()
{
 ...
 pad GetCurrentPad(double xpos, double ypos)

{
  ...
  cur_pad.row = row;
  cur_pad.layer = layer;
  cur_pad.x_inpad = x_inpad;
  cur_pad.z_inpad = y_inpad;
...
/*
void GetPadResponse(vector<hit> &hitarry, pad curPad, double charge)
{
  double x_step = 8;
  double y_step = 12;

  int row = curPad.row;
  int layer = curPad.layer;
  double x_0 = curPad.x_inpad;
  double y_0 = curPad.z_inpad;
	      ...
	      if(layer+1 < 113 && row+1<109)
		{
		  ...
		  cur_hit.row = row;
		  cur_hit.layer = layer;
		  cur_hit.charge = charge;
		  hitarry.push_back(cur_hit);
		}	    
	...
	for(int j = -1; j <= 1; j++)//layer
	  ...
	  if(row + i >= 0 && layer + j >= 0)
	      ...
	      if(layer+1 < 113 && row+1<109)
		{
		  hit cur_hit;
		  cur_hit.row = row + i;
		  cur_hit.layer = layer + i;
		  cur_hit.charge = charge*GetFract(x1,x2,y1,y2,x_0,y_0);
		  hitarry.push_back(cur_hit);
		}	    
	    }
	}
...
*/
void FillPadPlane(int row, int layer, double charge, vector<vector<double>> &padplane)
{
      if(layer+1 <113 && row+1<109)
	padplane.at(layer + 1).at(row + 1) += charge;
...
double GetC(vector<hit> &hitarray, double threshold, double trunc_f)
{
    std::sort(hitarray.begin(),hitarray.end(),by_layer_row);
  int cur_row = 0;
  int cur_layer = 0;
  vector<double> c_array;
  double layer_sum = 0, row_sum = 0;

  for(int iEl = 0; iEl < hitarray.size(); iEl++)
    {

      if(hitarray.at(iEl).layer < 0 || hitarray.at(iEl).layer > 112)
	continue;
      if(iEl == 0)
	{
	  cur_layer = hitarray.at(iEl).layer;
	  cur_row = hitarray.at(iEl).row;
	  layer_sum += hitarray.at(iEl).charge;
	}
      else
	{
	  if(cur_layer == hitarray.at(iEl).layer)
	    {
	      //In same layer now gather same row
	      layer_sum += hitarray.at(iEl).charge;
	    }
	  else
	    {
	      c_array.push_back(layer_sum/1.2);
	      //      cout<<"Layer sum at end of layer is "<<layer_sum<<endl;
	      layer_sum = hitarray.at(iEl).charge;
	      cur_layer = hitarray.at(iEl).layer;
	    }

	}
      //            cout<<"Layer sum is "<<layer_sum<<" for layer "<<cur_layer<<endl;
    }

  //  c_array.pop_back();//We apply +/- 1 layer to the current pad. So last pad is not a real segment
  std::sort(c_array.begin(),c_array.end(),by_double);

  int element = floor(trunc_f*c_array.size());
  c_array.erase(c_array.begin()+element,c_array.end());
  double sum = std::accumulate(c_array.begin(),c_array.end(),0.0);
  sum = sum/c_array.size();
...
double GetC(vector<vector<double>>padplane, double threshold, double trunc_f)
{
  ...
  for(int iLayer = 1; iLayer<= num_layers; iLayer++)
    ...
    int element = floor(trunc_f*chg_layers.size());
    //  if(element != 0)
...
int main()
{
  ...
  double part_mass = p_mass;
  double part_mom  = p_mom;
  ...
  TH1D *remainder = new TH1D("rem","rem",100,0,100);
  TH1D *  c_dist_p10 = p10.GetMCdist(bichsel_p10,"c_dist_p10",segment,ns,factor,1e6); 
  TH1D *test_mc = new TH1D("testmc","testmc",10000,1000,3500);
  TH1D * c_dist_fine = new TH1D("c_dist_fine","c_dist_fine",1e3,0,1e4);
  TH1D * c_dist = new TH1D("c_dist","c_dist",333,0,1e4);
  ...
  TGraph *check = new TGraph(cdf_y.size(), cdf_x.data(),cdf_y.data());
  ...
  for(int i = 0 ;i < cdf_y.size();i++)
    cout<<cdf_y.at(i)<<" "<<cdf_x.at(i)<<endl;

  //  TRandom3 *ran = new TRandom3(1234);
  TRandom3 *ran = new TRandom3(3894183412340);
  ...
  for(int iTrack = 0; iTrack < num_mc; iTrack++)
      ...
      for(int iSeg = 0 ;iSeg < ns; iSeg++)
	{
	  ...
	  //	  cout<<"eloss "<<e_loss<<endl;
	  int num_elec  = floor(e_loss/eVperelec);
	  double e_chg = (e_loss/eVperelec)/num_elec;
	  double rem = (e_loss-num_elec*e_chg*eVperelec);
	  remainder->Fill(rem);
	  //	  cout<<"remainder is "<<rem<<endl;
	  //	  cout<<"num_elec "<<num_elec<<endl;
	  //	  double stepsize = (segment*10)/(num_elec + 1); // add 1 for number of gaps between segment
	  //	  z_curr += stepsize;
	  for( int iEl = 0 ;iEl < num_elec; iEl++)
	    {
	      z_curr = ran -> Uniform(iSeg *(segment *10), (iSeg + 1)*(segment*10));
	      //	      double pos_inseg = stepsize * (iEl + 1); 
	      double dr = ran -> Gaus(0,sigmaT);
	      ...
	      pad cur_pad =  GetCurrentPad(drift_x,drift_z);
	      GetPadResponse(hitarray,cur_pad,1);
	      //  z_curr += stepsize;
	    }
	  //	  testmc->Fill(eloss);
	}//end of segment loop
      
      double c_eloss = GetC(hitarray,0,.7);
      //      cout<<"c eloss "<<c_eloss<<endl;
      c_dist->Fill(c_eloss*eVperelec);      
      }
  ...
  */
  ...
  c_dist->GetXaxis()->SetRangeUser(1e3,4e3);
  c_dist->Draw();
